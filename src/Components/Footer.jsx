...
const Footer = () => {
  return (
    <div className='relative flex gap-5 w-full min-h-screen bg-zinc-900 px-10 py-10'>
        <div className='w-1/2 h-full block justify-between' style={{ fontFamily: "var(--font-founders)" }}>
            <div className="headings">
                <h1 className='text-[9.5vw] uppercase leading-none -mb-6 tracking-wide'>Eye-</h1>
                <h1 className='text-[9.5vw] uppercase leading-none -mb-6 tracking-wide'>Opening</h1>
            </div>
            
            
        </div>
        <div className='w-1/2'>
            <h1 className='text-[9.5vw] uppercase leading-none -mb-6 tracking-wide' style={{ fontFamily: "var(--font-founders)" }}>Presentations</h1>
            <div className='dets mt-20'>
                <h1 className='mb-5'>S:</h1>
                {['Facebook','Instagram','Behance','linkedin'].map((items, index)=><a className='block text-lg font-light' src='#'>{items}</a>)}
                <br/>
                <h1 className='mb-5'>L:</h1>
                {['202-1965 W 4th Ave', 'Vancouver, Canada'].map((items, index)=><a className='block text-lg font-light' src='#'>{items}</a>)}
                <br/>
                {['30 Chukarina St','Lviv, Ukraine'].map((items, index)=><a className='block text-lg font-light' src='#'>{items}</a>)}
                
                <br />
                <h1 className='mb-3'>E:</h1>
                <a className='' src='#'>hello@ochi.design</a>
            </div>

            <svg  className=' absolute bottom-5 left-5' 
            width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
